...
class Model(ModelTemplate):
    def model_get(self):
        ...
        self.model = self.krs.Sequential([
            ...
            self.krs.layers.GRU(layer_list[0][1]['units'], return_sequences=False, kernel_initializer=initializer),
            self.krs.layers.Dense(layer_list[1][1]['units'], activation=layer_list[1][1]['activation'], kernel_initializer=initializer),
            self.krs.layers.Dropout(layer_list[2][1]['dropout']),
            self.krs.layers.Dense(layer_list[3][1]['units'], activation=layer_list[3][1]['activation'], kernel_initializer=initializer),
            self.krs.layers.Dropout(layer_list[4][1]['dropout']),
